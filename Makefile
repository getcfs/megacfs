...
build:
	mkdir -p $(BUILDPATH)
	go build -i -v -o build/ring github.com/getcfs/megacfs/ring
	go build -i -v -o build/cfs --ldflags "\
			-X main.commitVersion=$(SHA) \
			-X main.cfsVersion=$(VERSION) \
			-X main.goVersion=$(GOVERSION) \
			-X main.buildDate=$(BDATE)"\
			github.com/getcfs/megacfs/cfs
	go build -i -v -o build/cfsd --ldflags "\
			-X main.commitVersion=$(SHA) \
			-X main.cfsdVersion=$(VERSION) \
			-X main.goVersion=$(GOVERSION) \
			-X main.buildDate=$(BDATE)"\
			github.com/getcfs/megacfs/cfsd
	go build -i -v -o build/cfsadm --ldflags "\
			-X main.commitVersion=$(SHA) \
			-X main.cfsadmVersion=$(VERSION) \
			-X main.goVersion=$(GOVERSION) \
			-X main.buildDate=$(BDATE)"\
			github.com/getcfs/megacfs/cfsadm
