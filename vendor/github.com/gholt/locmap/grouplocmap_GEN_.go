...
// GroupLocMapConfig represents the set of values for configuring a
// GroupLocMap. Note that changing the values in this structure will have no
// effect on existing GroupLocMaps; they are copied on instance creation.
...
func resolveGroupLocMapConfig(c *GroupLocMapConfig) *GroupLocMapConfig {
	cfg := &GroupLocMapConfig{}
	if c != nil {
		*cfg = *c
	}
...
func (locmap *groupLocMap) split(n *groupLocMapNode) {
	...
	var xn *groupLocMapNode
	var yn *groupLocMapNode
	nes := n.entries
	lm := locmap.lowMask
	...
	for i := uint32(0); i <= lm; i++ {
		...
		for xe.next != 0 {
			...
			if yes == nil {
				yes = make([]groupLocMapEntry, len(xn.entries))
				yn.entries = yes
				...
				*ye = *xen
				...
				if yn.overflowLowestFree != 0 {
					...
					*ye2 = *xen
					...
					yo = append(yo, make([]groupLocMapEntry, 1<<b))
					yn.overflow = yo
					if yoc == 0 {
						ye2 := &yo[0][1]
						*ye2 = *xen
