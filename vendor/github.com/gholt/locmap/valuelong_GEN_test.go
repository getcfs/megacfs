...
func TestValueExerciseSplitMergeLong(t *testing.T) {
	...
	locmap := NewValueLocMap(&ValueLocMapConfig{Roots: 8, PageSize: 512, SplitMultiplier: 1}).(*valueLocMap)
	...
	for i := 0; i < keysetCount; i++ {
		go func(j int) {
			...
			if j%100 == 33 {
				uselessCounter := 0
				stopped, more := locmap.ScanCallback(0, math.MaxUint64, 0, 0, math.MaxUint64, math.MaxUint64, func(keyA uint64, keyB uint64, timestamp uint64, length uint32) bool {
					uselessCounter++
					return true
				})
				if more {
