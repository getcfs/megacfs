...
func TestGroupScanCallbackBasic(t *testing.T) {
	...
	good := false
	stopped, more := locmap.ScanCallback(0, math.MaxUint64, 0, 0, math.MaxUint64, 100, func(keyA2 uint64, keyB2 uint64, childKeyA2 uint64, childKeyB2 uint64, timestamp2 uint64, length2 uint32) bool {
...
func TestGroupScanCallbackRangeMiss(t *testing.T) {
	...
	locmap.Set(keyA1, keyB1, 0, 0, timestamp1, blockID1, offset1, length1, false)
	good := false
	stopped, more := locmap.ScanCallback(101, math.MaxUint64, 0, 0, math.MaxUint64, 100, func(keyA2 uint64, keyB2 uint64, childKeyA2 uint64, childKeyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		t.Fatalf("%x %x %d %d\n", keyA2, keyB2, timestamp2, length2)
		return false
	})
	if good {
	...
	good = false
	stopped, more = locmap.ScanCallback(0, 99, 0, 0, math.MaxUint64, 100, func(keyA2 uint64, keyB2 uint64, childKeyA2 uint64, childKeyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		t.Fatalf("%x %x %d %d\n", keyA2, keyB2, timestamp2, length2)
		return false
	})
	if good {
...
func TestGroupScanCallbackMask(t *testing.T) {
	...
	locmap.Set(keyA1, keyB1, 0, 0, timestamp1, blockID1, offset1, length1, false)
	good := false
	stopped, more := locmap.ScanCallback(0, math.MaxUint64, 1, 0, math.MaxUint64, 100, func(keyA2 uint64, keyB2 uint64, childKeyA2 uint64, childKeyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		if keyA2 == keyA1 && keyB2 == keyB1 {
			if timestamp2 != timestamp1 {
				t.Fatal(timestamp2)
			}
	...
	good = false
	stopped, more = locmap.ScanCallback(0, math.MaxUint64, 2, 0, math.MaxUint64, 100, func(keyA2 uint64, keyB2 uint64, childKeyA2 uint64, childKeyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		t.Fatalf("%x %x %d %d\n", keyA2, keyB2, timestamp2, length2)
		return false
	})
	if good {
...
func TestGroupScanCallbackNotMask(t *testing.T) {
	...
	locmap.Set(keyA1, keyB1, 0, 0, timestamp1, blockID1, offset1, length1, false)
	good := false
	stopped, more := locmap.ScanCallback(0, math.MaxUint64, 0, 2, math.MaxUint64, 100, func(keyA2 uint64, keyB2 uint64, childKeyA2 uint64, childKeyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		if keyA2 == keyA1 && keyB2 == keyB1 {
			if timestamp2 != timestamp1 {
				t.Fatal(timestamp2)
			}
	...
	good = false
	stopped, more = locmap.ScanCallback(0, math.MaxUint64, 0, 1, math.MaxUint64, 100, func(keyA2 uint64, keyB2 uint64, childKeyA2 uint64, childKeyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		t.Fatalf("%x %x %d %d\n", keyA2, keyB2, timestamp2, length2)
		return false
	})
	if good {
...
func TestGroupScanCallbackCutoff(t *testing.T) {
	...
	locmap.Set(keyA1, keyB1, 0, 0, timestamp1, blockID1, offset1, length1, false)
	good := false
	stopped, more := locmap.ScanCallback(0, math.MaxUint64, 0, 0, 123, 100, func(keyA2 uint64, keyB2 uint64, childKeyA2 uint64, childKeyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		if keyA2 == keyA1 && keyB2 == keyB1 {
			if timestamp2 != timestamp1 {
				t.Fatal(timestamp2)
			}
	...
	good = false
	stopped, more = locmap.ScanCallback(0, math.MaxUint64, 0, 0, 122, 100, func(keyA2 uint64, keyB2 uint64, childKeyA2 uint64, childKeyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		t.Fatalf("%x %x %d %d\n", keyA2, keyB2, timestamp2, length2)
		return false
	})
	if good {
...
func TestGroupScanCallbackMax(t *testing.T) {
	...
	count := 0
	stopped, more := locmap.ScanCallback(0, math.MaxUint64, 0, 0, math.MaxUint64, 50, func(keyA2 uint64, keyB2 uint64, childKeyA2 uint64, childKeyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		count++
		return true
	})
	if count != 50 {
	...
	count = 0
	stopped, more = locmap.ScanCallback(0, math.MaxUint64, 0, 0, math.MaxUint64, 5000, func(keyA2 uint64, keyB2 uint64, childKeyA2 uint64, childKeyB2 uint64, timestamp2 uint64, length2 uint32) bool {
		count++
		return true
	})
	if count != 4000 {
...
func TestGroupStatsBasic(t *testing.T) {
...
func TestGroupExerciseSplitMergeDiscard(t *testing.T) {
	// count needs to be high enough to fill all the root pages, hit the
	// overflow of those pages, and some pages below that too.
	count := 100000
	// seed just provides a repeatable test scenario.
