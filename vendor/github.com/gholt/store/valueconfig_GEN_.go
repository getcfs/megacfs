...
const _VALUE_PAGE_SIZE_MIN = 8 + _VALUE_FILE_ENTRY_SIZE

// ValueStoreConfig represents the set of values for configuring a
// ValueStore. Note that changing the values (shallow changes) in this
// structure will have no effect on existing ValueStores.
...
func resolveValueStoreConfig(c *ValueStoreConfig) *ValueStoreConfig {
	...
	if cfg.PageSize < _VALUE_PAGE_SIZE_MIN {
		cfg.PageSize = _VALUE_PAGE_SIZE_MIN
	}
	...
	cfg.minValueAlloc = cfg.ChecksumInterval/(cfg.PageSize/_VALUE_FILE_ENTRY_SIZE+1) + 1
	...
	if env := os.Getenv("VALUESTORE_FILE_CAP"); env != "" {
		if val, err := strconv.Atoi(env); err == nil {
			cfg.FileCap = val
		}
	}
	...
	if cfg.FileCap < _VALUE_FILE_HEADER_SIZE+_VALUE_FILE_TRAILER_SIZE+cfg.ValueCap { // header value trailer
		cfg.FileCap = _VALUE_FILE_HEADER_SIZE + _VALUE_FILE_TRAILER_SIZE + cfg.ValueCap
	}
	if cfg.FileCap > math.MaxUint32 {
		cfg.FileCap = math.MaxUint32
	}
	if env := os.Getenv("VALUESTORE_FILE_READERS"); env != "" {
		if val, err := strconv.Atoi(env); err == nil {
			cfg.FileReaders = val
		}
	}
	...
	if env := os.Getenv("VALUESTORE_PUSH_REPLICATION_INTERVAL"); env != "" {
		if val, err := strconv.Atoi(env); err == nil {
	...
	if env := os.Getenv("VALUESTORE_PUSH_REPLICATION_WORKERS"); env != "" {
		if val, err := strconv.Atoi(env); err == nil {
	...
	if env := os.Getenv("VALUESTORE_PUSH_REPLICATION_MSG_TIMEOUT"); env != "" {
		if val, err := strconv.Atoi(env); err == nil {
	...
	if cfg.FlusherThreshold == 0 {
		cfg.FlusherThreshold = int32(cfg.PageSize) / _VALUE_FILE_ENTRY_SIZE
	}
	if cfg.FlusherThreshold < 0 {
		cfg.FlusherThreshold = 0
	}
	if env := os.Getenv("VALUESTORE_AUDIT_INTERVAL"); env != "" {
		if val, err := strconv.Atoi(env); err == nil {
	...
	if env := os.Getenv("VALUESTORE_AUDIT_AGE_THRESHOLD"); env != "" {
		if val, err := strconv.Atoi(env); err == nil {
