...
func (store *defaultValueStore) watcherConfig(cfg *ValueStoreConfig) {
...
func (store *defaultValueStore) watcherStartup() {
...
func (store *defaultValueStore) watcherShutdown() {
	store.watcherState.startupShutdownLock.Lock()
	if store.watcherState.notifyChan != nil {
...
func (store *defaultValueStore) watcherLauncher(notifyChan chan *bgNotification) {
	...
	for running {
		...
		if wantToDisable != "" {
			if !disabled {
				store.logger.Error(wantToDisable, zap.String("name", store.loggerPrefix+"watcher"))
				store.disableWrites(false) // false indicates non-user call
				disabled = true
			}
		} else if wantToReenable != "" {
			if disabled {
				store.logger.Error(wantToReenable, zap.String("name", store.loggerPrefix+"watcher"))
